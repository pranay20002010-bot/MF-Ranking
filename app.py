...
def make_model(name: str = "Ridge", ridge_alpha: float = 50.0, **_):
    if name != "Ridge":
...
def build_pdf_report(s: dict) -> bytes:
    ...
    boxes = [("Funds ranked", str(s["n_funds"])), ("Looking ahead", f"{s['horizon']} months"),
             ...
             ("Ranking 1 evidence", s["ev1"]), ("Ranking 2 evidence", s["ev2"])]
    n = len(boxes)
    ...
    story += [cream([Paragraph(f"<b>Ranking 1 (observed performance).</b> {_safe(' '.join(s['points1']))}", spaced),
...
R1, R2 = "Composite (normalise & rank)", "Ridge"        # internal keys: Ranking 1 (observed), Ranking 2 (regression)
LABELS = {R1: "Ranking 1: observed performance", R2: "Ranking 2: panel regression",
...
def method_summary(score: pd.DataFrame, method: str, horizon: int) -> dict:
    ...
    vs_simple = None
    if method == R2 and pd.notna(simple_edge):
        diff = edge - simple_edge
        vs_simple = ("Compared with Ranking 1 (observed performance), the regression ranking was "
...
def make_report_summary(cat, asof, horizon, t, yg, ms1, ms2, score, cols, win_months, n_dates, reg, reg_tb, fit_error, bnote):
    ...
    summ = {
        ...
        "ev1": ms1["label"] if ms1 else "n/a", "ev2": ms2["label"] if ms2 else "n/a",
        "intro": [f"This note compares {len(t)} Direct-Growth {cat} mutual funds. Ranking 1 ranks them on what they actually delivered "
                  ...
                  f"numbers. Neither ranking is a forecast for any single fund."],
        ...
        "method_edges": [(pretty(m), float(score.loc[m, "top_vs_avg"])) for m in [R1, R2, "Trailing 12M return", "Sharpe only"]
...
returns) and produces **two rankings inside each category**:
...
- **Ranking 2: panel regression (forward-looking).** A regression learns from history which of these measures were followed by
  better results versus peers, then applies those lessons to today's numbers. It is the *only* forecasting model in the app.

We then test both honestly on past data (the **Have these worked before?** tab). If neither has a track record, the app says so.

### The tabs
1. **Rankings**: both rankings side by side, plus **newer funds** that are too young for the full window (see below).
   The button creates the VIKA-branded PDF.
2. **Have these worked before?**: replays history month by month with no peeking.
3. **Regression results**: what the regression found: which metrics it leans on, how confident we can be, how well it fits.
...
def get_backtest(_df, src, cats, horizon, method, features, target, min_train, step, alpha):
    return walk_forward(_df, list(features), horizon, [R2], target,
                        min_train_months=min_train, step=step, ridge_alpha=alpha)


@st.cache_data(show_spinner="Fitting the regression…")
def get_latest(_df, src, cats, horizon, method, features, target, alpha):
    out = rank_latest(_df, list(features), horizon, R2, target, alpha)
...
cols = [R2] + list(BASELINES)                       # R1 is one of the BASELINES
...
with tab_rank:
    ...
    fit_error = None
    try:
        latest, attrs = get_latest(df, src, cats_t, horizon, method, feats_t, target, alpha)
    except ValueError as e:                     # e.g. not enough history to train: Ranking 2 unavailable
        fit_error = str(e)
    ...
    t = t.sort_values("rank_model" if order.startswith("Ranking 2") else "rank_composite")
    show = pd.DataFrame({
    ...
    if fit_error:
        show = show.drop(columns=["Rank 2 (regression)"])
    ...
    full = t[["rank_composite", "rank_model", "scheme_name", "fund_house", "strengths", "watchouts", "score"]
    ...
    if ms1:
        try:
            ...
            pdf = build_pdf_report(make_report_summary(
                ...
                reg_table(rep) if rep else None, fit_error, benchmark_note(df, cat)))
...
with tab_bt:
    if bt_error:
        ...
        bar_df["Type"] = np.where(bar_df["model"].isin([R1, R2]), "Our rankings", "Simple rule")
        ...
        ts = pd.DataFrame({pretty(R1): top_bucket_series(preds, R1), pretty(R2): top_bucket_series(preds, R2)}
                          ).rolling(12, min_periods=6).mean()
        figt = px.line(ts, labels={"value": "Top-20% edge (12-month average)", "date": "", "variable": ""},
                       color_discrete_sequence=["#F6BA47", "#534191"])
        ...
        with st.expander("By category"):
            bc = score_predictions(preds, [R1, R2], horizon, by_category=True).reset_index()
