...
CATEGORIES: dict[str, dict] = {
    ...
    "Aggressive Hybrid": {
        "match": ["hybrid scheme - aggressive hybrid", "aggressive hybrid fund"],
        "bench": "HYBRID_AGG",
    },
    "Multi Asset Allocation": {
        "match": ["hybrid scheme - multi asset", "multi asset allocation"],
        "bench": "MULTI_ASSET",
    },
}
...
BENCHMARKS: dict[str, dict] = {
    ...
    # Building blocks for the blended hybrid benchmarks below. `no_fallback` = never substitute the Nifty 50 ETF for these.
    "GOLD": {"name": "Gold (Gold BeES ETF)", "tickers": ["GOLDBEES.NS"], "no_fallback": True},
    "DEBT": {"name": "Debt proxy (5-yr G-Sec ETF)", "tickers": ["GILT5YBEES.NS", "LTGILTBEES.NS"], "no_fallback": True,
             "accrual_fill": 0.07},        # before the ETF lists (or if it fails) the debt leg accrues at 7% a year
    # Blends: constant weights, rebalanced daily. There is no Yahoo ticker for the official hybrid indices, so these are
    # proxies you can edit (weights are the usual ballpark for each category).
    "HYBRID_AGG": {"name": "65:35 Equity/Debt blend", "blend": [("NIFTY500", 0.65), ("DEBT", 0.35)]},
    "MULTI_ASSET": {"name": "50:30:20 Equity/Debt/Gold blend", "blend": [("NIFTY500", 0.50), ("DEBT", 0.30), ("GOLD", 0.20)]},
}
...
_EXCLUDE = re.compile(
    ...
    r"liquid|overnight|gilt|money market|fixed maturity|fmp|fixed term|"
...
# 'debt' / 'bond' in a name means a pure debt fund -- unless it is a hybrid ("Equity & Debt Fund", "Equity & Bond Fund")
_DEBT_WORDS = re.compile(r"debt|bond", re.I)
_HYBRID_WORDS = re.compile(r"equity|hybrid|balanced|multi|asset|allocation|advantage", re.I)
...
def direct_growth_candidates(schemes: pd.DataFrame) -> pd.DataFrame:
    ...
    keep &= ~(n.str.contains(_DEBT_WORDS) & ~n.str.contains(_HYBRID_WORDS))
...
def _load_simple(key: str, spec: dict, start: str) -> tuple[pd.Series, str]:
    """CSV in ./benchmarks/ wins; else try each ticker in order, then the last-resort ETF proxy."""
    ...
    tried = []
    chain = list(spec["tickers"]) + ([] if spec.get("no_fallback") else [LAST_RESORT_PROXY])
    for tk in chain:
        if tk in tried:
            continue
        tried.append(tk)
...
def _build_blend(key: str, spec: dict, start: str, memo: dict) -> tuple[pd.Series, str]:
    """Constant-weight blend of other benchmarks, rebalanced daily. A component with `accrual_fill` (the debt leg)
    accrues at that rate wherever its own data is missing, so the blend keeps the equity leg's full history."""
    parts = []
    for ck, w in spec["blend"]:
        cspec = BENCHMARKS[ck]
        try:
            ser, src = load_benchmark(ck, start, memo)
        except ValueError:
            if "accrual_fill" not in cspec:
                raise
            ser, src = None, f"{cspec['accrual_fill']:.1%} accrual"
        parts.append((ck, w, ser, src))
    base = next((p[2] for p in parts if p[2] is not None), None)
    if base is None:
        raise ValueError(f"No component of blend {key} could be loaded")
    cal = base.index
    rets, weights = {}, {}
    for ck, w, ser, _ in parts:
        r = (pd.Series(np.nan, index=cal) if ser is None else
             ser.reindex(cal.union(ser.index)).ffill(limit=5).reindex(cal).pct_change(fill_method=None))
        acc = BENCHMARKS[ck].get("accrual_fill")
        if acc is not None:
            r = r.fillna((1 + acc) ** (1 / 252) - 1)
        rets[ck], weights[ck] = r, w
    R = pd.DataFrame(rets).iloc[1:].dropna()
    ret = sum(R[ck] * w for ck, w in weights.items())
    level = 100 * (1 + ret).cumprod()
    level.name = key
    label = " + ".join(f"{w:.0%} {BENCHMARKS[ck]['name']} ({src})" for ck, w, _, src in parts)
    return level, label


def load_benchmark(key: str, start: str = "2005-01-01", _memo: dict | None = None) -> tuple[pd.Series, str]:
    """(daily level series, source label). Raises only if every option fails."""
    memo = {} if _memo is None else _memo
    if key in memo:
        return memo[key]
    spec = BENCHMARKS[key]
    out = _build_blend(key, spec, start, memo) if "blend" in spec else _load_simple(key, spec, start)
    memo[key] = out
    return out


def load_benchmarks(keys: list[str], start: str = "2005-01-01", strict: bool = False) -> pd.DataFrame:
    """Benchmark levels for `keys`. df.attrs['sources'] = what was actually used, df.attrs['errors'] = keys that failed.
    Unless strict, one failing benchmark does not stop the others (categories using it are simply skipped)."""
    series, sources, errors, memo = {}, {}, {}, {}
    for k in keys:
        try:
            series[k], sources[k] = load_benchmark(k, start, memo)
        except Exception as e:
            if strict:
                raise
            errors[k] = str(e)
    if not series:
        raise ValueError("No benchmark could be loaded: " + "; ".join(f"{k}: {v}" for k, v in errors.items()))
    df = pd.concat(series, axis=1).sort_index()
    df.attrs["sources"], df.attrs["errors"] = sources, errors
    return df
...
# --------------------------------------------------------------------------- #
# Every row of the newer-funds table carries ALL of these columns (NaN where not applicable), even when no fund is old
# enough to be ranked -- downstream code sorts / formats on them unconditionally.
NEW_FUND_COLS = ["date", "scheme_code", "category", "kind", "history_months", "since_launch", "benchmark", "bench_source",
                 "sharpe", "sortino", "treynor", "info_ratio", "alpha", "beta", "up_capture", "down_capture",
                 "capture_ratio", "volatility", "max_drawdown", "ann_return", "lfl_rank", "lfl_peers"]
...
def compute_new_funds(
    ...
    for cat, spec in CATEGORIES.items():
        ...
        for j, code in enumerate(codes):
            ...
            out.append(row)
    return pd.DataFrame(out).reindex(columns=NEW_FUND_COLS)
...
def demo_universe(funds_per_category: int = 25, start: str = "2008-01-01", end: str = "2026-08-31",
    ...
    for key in BENCHMARKS:
        load = {"NIFTY50": 1.0, "NIFTY100": 1.0, "NIFTY500": 1.03, "NIFTYMID150": 1.2,
                "NIFTYSMALL250": 1.3, "NIFTYLMC250": 1.1, "HYBRID_AGG": 0.7, "MULTI_ASSET": 0.5,
                "GOLD": 0.15, "DEBT": 0.05}.get(key, 1.0)
    ...
    for cat in cats:
        ...
        base_beta = {"Balanced Advantage (BAF)": 0.55, "Aggressive Hybrid": 0.80, "Multi Asset Allocation": 0.60}.get(cat, 0.95)
...
def benchmark_note(df: pd.DataFrame, cat: str) -> str:
    ...
    name = spec["name"]
    if "blend" in spec:
        return (f"{cat} funds are compared with a blended proxy benchmark, {name}, rebalanced daily: {srcname}. "
                "(There is no listed index for this category's official benchmark.)")
    if srcname in ("", "nan"):
        return f"{cat} funds are compared with the {name}."
    if srcname == spec["tickers"][0] or srcname.startswith("CSV"):
...
- **Hybrid benchmarks.** Aggressive Hybrid is compared with a 65:35 equity/debt blend and Multi Asset Allocation with a 50:30:20
  equity/debt/gold blend, built from listed ETFs (the official hybrid indices have no Yahoo ticker). Weights are editable in the CONFIG section.
...
def live_build(cats: tuple, freq: str, window: int, progress=None):
    ...
    if hit and time.time() - hit[0] < 24 * 3600:
        st.session_state["bench_errors"] = hit[3]
        return hit[1], hit[2]
    ...
    errors = dict(bench.attrs.get("errors", {}))
    st.session_state["bench_errors"] = errors
    ...
    store[key] = (time.time(), raw, meta, errors)
    while len(store) > 2:
...
with st.sidebar:
    ...
    if choice.startswith("Saved"):
        ...
        try:
            ...
            st.stop()
        bar.empty()
        for bk, msg in st.session_state.get("bench_errors", {}).items():
            skipped = [c for c in st.session_state["live_args"][0] if CATEGORIES[c]["bench"] == bk]
            if skipped:
                st.warning(f"Benchmark '{BENCHMARKS[bk]['name']}' could not be loaded, so {', '.join(skipped)} "
                           "were left out. Check the Yahoo tickers in the CONFIG section, or add a Date,Close CSV in benchmarks/.")
    ...
    for col_ in NEW_FUND_COLS:                  # files built before this fix may lack some (e.g. no fund was old enough to rank)
        if col_ not in young_all.columns:
            young_all[col_] = np.nan
